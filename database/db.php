...
class DB {
	...
	public static function addUser($loginName, $displayName, $homeDir, $pid) {
		if (self::loginNameExists($loginName)) {
			...
			$stmt = \OC::$server->getDatabaseConnection()->prepare('UPDATE *PREFIX*shibboleth_user SET deleted_on = "" WHERE login_name = ?');
			$result = $query->stmt(array($loginName));
		} else {
			\OCP\Util::writeLog(APP_NAME, "adding user: $loginName", \OCP\Util::INFO);
			$stmt = \OC::$server->getDatabaseConnection()->prepare('INSERT INTO *PREFIX*shibboleth_user values(?, ?, ?, ?, datetime("now"), "")');
			$result = $query->stmt(array($loginName, $displayName, $homeDir, $pid));
		}
		return !($result === false);
	}
